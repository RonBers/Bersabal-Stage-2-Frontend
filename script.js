async function fetchData() {
  try {
    ...
    const defaultCardContent = users.data.map((user) => {
      return `
    ...
    const customCardContent = users.data.map((user) => {
      return `
    ...
    cardContainer.innerHTML = defaultCardContent.join("")
    cardContainer2.innerHTML = customCardContent.join("")
  } catch (error) {
